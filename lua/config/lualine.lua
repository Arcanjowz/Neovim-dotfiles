local function config_lualine(colors)
  ...
  -- OBS: o indicador de macro gravando via noice.api.status.mode foi removido
  -- daqui. Não existe nenhum spec de "folke/noice.nvim" em lua/plugins/ (só
  -- aparecia no lazy-lock.json, sobra de uma versão antiga da config), então
  -- esse require("noice") direto (sem pcall) quebraria a statusline inteira
  -- assim que o plugin fosse removido do disco (ex.: em :Lazy clean). Se você
  -- quiser esse indicador de volta, instale o plugin de verdade primeiro:
  --
  --   { "folke/noice.nvim", event = "VeryLazy", dependencies = { "MunifTanjim/nui.nvim" }, opts = {} }
  --
  -- e então reintroduza o bloco `macro` (usando require("noice"), já que a
  -- dependência vai existir de fato) e adicione `macro` de volta em lualine_y.
