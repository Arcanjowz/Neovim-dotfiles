...
--- Paleta oldworld.nvim (hex oficiais, mesmos do dgox16/devicon-colorscheme.nvim)
...
--- Cria (ou recria) um highlight group com fg custom + o bg sólido do popup,
--- e devolve o nome do grupo pronto pra usar em border_hl/name_hl.
---@param suffix string
---@param fg string
---@return string
local function hl(suffix, fg)
  local group = "LspHover" .. suffix
  vim.api.nvim_set_hl(0, group, { fg = fg, bg = oldworld.bg_dark, bold = true })
  return group
end

--- Fundo sólido para o popup inteiro (texto + borda), pra ficar mais visível
--- do que o NormalFloat padrão (que costuma ser transparente/translúcido
--- dependendo do terminal). Reaplica sempre que o colorscheme mudar.
local function set_popup_background()
  vim.api.nvim_set_hl(0, "LspHoverNormal", { bg = oldworld.bg_dark, fg = oldworld.white })
end
set_popup_background()
vim.api.nvim_create_autocmd("ColorScheme", { callback = set_popup_background })
...
lsp_hover.config = {
  default = {
    border_hl = hl("Default", oldworld.white),
    name = "\u{f085} LSP/Hover", -- codicon: gear
  ...
  ["^lua_ls"] = {
    name = " LuaLS", -- codicon: symbol-namespace
    border_hl = hl("Lua", oldworld.blue),
  },

  ["^html$"] = {
    name = " HTML", -- codicon: code
    border_hl = hl("Html", oldworld.orange),
  },

  ["^cssls$"] = {
    name = " CSS", -- codicon: symbol-color
    border_hl = hl("Css", oldworld.purple),
  },

  ["^emmet"] = {
    name = "\u{f13b} Emmet",
    border_hl = hl("Emmet", oldworld.orange),
  },

  ["^ts_ls$"] = {
    name = "  TS/JS", -- codicon: symbol-method
    border_hl = hl("Ts", oldworld.blue),
  },

  ["^biome$"] = {
    name = "\u{f0459} Biome", -- codicon: symbol-misc
    border_hl = hl("Biome", oldworld.blue),
  },

  ["^basedpyright$"] = {
    name = "\u{f0320} Python", -- codicon: symbol-class
    border_hl = hl("Python", oldworld.yellow),
  },

  ["^ruff$"] = {
    name = "\u{f0602} Ruff", -- codicon: symbol-event
    border_hl = hl("Ruff", oldworld.yellow),
  },

  ["^clangd$"] = {
    name = "\u{f0295} C/C++", -- codicon: symbol-struct
    border_hl = hl("Clangd", oldworld.cyan),
  },

  ["^gopls$"] = {
    name = "\u{f0669} Go", -- codicon: symbol-module
    border_hl = hl("Go", oldworld.cyan),
  },

  ["^rust_analyzer$"] = {
    name = "\u{f0432} Rust", -- codicon: symbol-constant
    border_hl = hl("Rust", oldworld.orange),
  },

  ["^sqlls$"] = {
    name = "\u{f01c2} SQL", -- codicon: database
    border_hl = hl("Sql", oldworld.green),
  },

  ["^bashls$"] = {
    name = "\u{f120} Bash", -- codicon: terminal
    border_hl = hl("Bash", oldworld.green),
  },

  ["^yamlls$"] = {
    name = "\u{f0e9c} YAML", -- codicon: symbol-key
    border_hl = hl("Yaml", oldworld.magenta),
  },

  ["^docker"] = {
    name = "\u{f0868} Docker", -- codicon: symbol-namespace (variant)
    border_hl = hl("Docker", oldworld.blue),
  },
}
...
lsp_hover.hover = function(error, result, context, _)
  ...
  -- Fundo sólido pro popup: sem isso o Normal/NormalFloat padrão do
  -- colorscheme pode ficar translúcido/misturado com o texto por trás,
  -- dependendo do terminal. LspHoverNormal é definido no topo do arquivo
  -- com o bg_dark da paleta oldworld.
  vim.wo[lsp_hover.window].winhighlight = "Normal:LspHoverNormal,NormalFloat:LspHoverNormal,FloatBorder:"
    .. (config.border_hl or "LspHoverDefault")
